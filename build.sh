...
source "scripts/get-cc-flags.sh" || exit 1
set -e || exit 1
...
srcArchiveName="JINJ-Source-$VERSION"
develArchiveName="JINJ-Devel-$VERSION"
objArchiveName="JINJ-ObjFiles-$VERSION"
fullArchiveName="JINJ-Full-$VERSION"
...
Help() {
    ...
    Sep
    PrintOption "--cc=<compiler>"              "Specify the C compiler to use (default: cc)"
    PrintOption "--ccflags=<flags>"            "Specify the compiler flags (default: -Wall -Wextra -std=c++20 -O2 -fPIC)"
...
CompileSources() {
    mapfile -t cFiles < <(find "$SRC" -name "*.c")
    objFiles=()
    ...
    for file in "${toCompile[@]}"; do
        obj="$BUILD/$(basename "$file" .c).o"
        "$CC" "${CCFLAGS[@]}" "${includePath[@]/#/-I}" -c "$file" -o "$obj" || ShowError $CompilationErrorExit "Failed to compile $file"
...
BuildShared() {
    "$CC" "${CCFLAGS[@]}" -shared -o "$OUTDIR/$OUTPUT.so" "${objFiles[@]}" || ShowError $LinkingErrorExit "Failed to link."
}
